...
set_title(f"Venom Tool {VERSIONETOOL} - Delete Emojis")
clear()
...
async def on_ready():
    ...
    for emoji in serverr.emojis:
        try:
            await emoji.delete()
            print(f'{y}[{b}Venom Tool{y}]{g} Deleted {emoji} {w}')
        except Forbidden:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {emoji} [Missing permissions]{w}')
        except:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {emoji}{w}')
