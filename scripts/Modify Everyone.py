...
set_title(f"Venom Tool {VERSIONETOOL} - Modify Everyone")
clear()
    ...
    for role in serverr.roles:
        if role.name == "@everyone":
            try:
                ...
                print(f'{y}[{b}Venom Tool{y}]{g} Modified @everyone {w}')
            except Forbidden:
                print(f'{y}[{b}Venom Tool{y}]{r} Unable to modify @everyone [Missing permissions]{w}')
            except:
                print(f'{y}[{b}Venom Tool{y}]{r} Unable to modify @everyone{w}')
