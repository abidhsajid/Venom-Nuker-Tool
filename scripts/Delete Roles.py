...
set_title(f"Venom Tool {VERSIONETOOL} - Delete Roles")
clear()
...
async def on_ready():
    ...
    for role in serverr.roles:
        try:
            await role.delete()
            print(f'{y}[{b}Venom Tool{y}]{g} Deleted {role}{w}')
        except Forbidden:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {role} [Missing permissions]{w}')
        except:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {role}{w}')
