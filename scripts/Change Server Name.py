...
set_title(f"Venom Tool {VERSIONETOOL} - Change Server Name")
clear()
...
async def on_ready():
    ...
    try:
        await serverr.edit(name="NUKED BY DADDY J4STER", community=False)
        print(f'{y}[{b}Venom Tool{y}]{g} Modified server name{w}')
    except Forbidden:
        print(f'{y}[{b}Venom Tool{y}]{r} Unable to modify server name [Missing permissions]{w}')
    except:
        print(f'{y}[{b}Venom Tool{y}]{r} Unable to modify server name{w}')
