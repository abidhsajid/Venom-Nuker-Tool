...
set_title(f"Venom Tool {VERSIONETOOL} - Delete Channels")
clear()
...
async def on_ready():
    ...
    for channel in serverr.channels:
        try:
            ...
            print(f'{y}[{b}Venom Tool{y}]{g} Deleted {channel}{w}')
        except Forbidden:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {channel} [Missing permissions]{w}')
        except:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to delete {channel}{w}')
