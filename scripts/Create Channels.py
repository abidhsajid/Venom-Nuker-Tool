...
set_title(f"Venom Tool {VERSIONETOOL} - Create Channels")
clear()
filetoken = open("token.txt", "r")
token = filetoken.read()

channel_names = ['Nuked by TXT jaster', 'HAHAHAHAHA']
...
async def on_ready():
    ...
    for i in range(500):
        ...
        try:
            created_channel = await serverr.create_text_channel(channel_name)
            print(f'{y}[{b}Venom Tool{y}]{g} Created {created_channel}{w}')
        except Forbidden:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to create channel [Missing permissions]{w}')
        except:
            print(f'{y}[{b}Venom Tool{y}]{r} Unable to create channel{w}')
